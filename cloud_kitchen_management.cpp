...
public:
  ...
  void placeOrder() // function to place an order
  {
    ...
    if (payChoice == 1) // online payment
{
    transactionCounter++;  // increment before using
...
else // cash on delivery
{
    ...
    transactionId = ""; // optional for COD
}
    ...
    while (true)                                                       // while loop to allow user to confirm, add more items, or cancel the order
    {
      ...
      if (action == 1) // if user chooses to confirm the order
      {
        ...
        // Save updated transactionCounter
    ofstream fout("counter.txt");
    fout << transactionCounter;
    fout.close();
...
int main()
{
	SetConsoleOutputCP(CP_UTF8); // to print emojis

  CloudKitchen app;
  // Inside main(), after creating CloudKitchen app
ifstream fin("counter.txt");
if (fin >> app.transactionCounter) {
    // loaded successfully
}
fin.close();
  ...
  app.addKitchen(Kitchens(" 🌾 Rice Dice     ", {"Veg Biryani", "Chicken Biryani", "Egg Biryani", "Paneer Pulao", "Veg Fried Rice", "Raita", "Papad", "Sweet Lassi"}, {180, 220, 190, 200, 170, 50, 30, 70}));
